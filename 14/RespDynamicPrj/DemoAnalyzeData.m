...
if gXCntAxes1 > MAX_X_CNT % 判断是否接收满一屏数据
    % 接收满一屏数据时计算呼吸率，并更新窗口显示
    rr = CalcRespRate(gDataAxes1);
    if rr ~= -100
        set(gHandlesMFig.text_rr, 'String', num2str(rr)); % 正常值
    else
        set(gHandlesMFig.text_rr, 'String', '---'); % -100代表无效值
    end
...
if rem(gXCntAxes1, 10) == 0       
    ...
    gDataAxes2 = SmoothFilterResp(gDataAxes1); % 对原始的呼吸信号进行平滑滤波
    ...
    gDataAxes3 = FiltResp(gDataAxes1); % 使用IIR或FIR滤波器进行滤波         
    ...
    [x, y] = CalcAmpSpec(gDataAxes1); % 计算呼吸信号的归一化幅度谱
    gAxes4X = x; % 横坐标为频率值，单位为Hz
    gAxes4Y = y; % 纵坐标为归一化幅度值，最大值为1 
end 
